...
// CORREZIONE: Inizializzazione stabile della root di React.
// Viene creata una sola volta e riutilizzata.
if (rootElement) {
  if (!rootElement._reactRoot) { // Usa una proprietà custom per evitare conflitti
...
const ProductCard = ({ product, navigateTo, type }) => {
  return React.createElement(
    "div",
    {
      ...
      onClick: () => navigateTo(type === 'book' ? 'bookDetail' : 'productDetail', product)
    },
    React.createElement(
      "div",
...
// Componente HomePage (Aggiunto perché mancava)
...
const App = () => {
  ...
  console.log("DEBUG: Componente App avviato per il rendering.");
