...
// Inizio del tuo codice originale da bundle.js
// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
...
if (rootElement && !rootElement._reactRootContainer) {
  ...
  reactRoot = rootElement._reactRootContainer._internalRoot.current.child.stateNode.current;
} else {
...
// ... (Tutti i tuoi componenti React originali seguono qui) ...
// Esempio:
...
// ... (e tutti gli altri tuoi componenti: FilterBar, DetailPage, CollectionPage, etc.)
    ...
    const initializeFirebase = async () => {
      try {
        ...
        if (!firebaseConfig || !firebaseConfig.apiKey || firebaseConfig.apiKey === "YOUR_FIREBASE_API_KEY_HERE") {
          console.error("Firebase ERROR: Configurazione mancante o chiave API non valida.");
          setIsAuthReady(true);
          return;
        }

        if (!window.firebase || !window.firebase.initializeApp || !window.firebase.auth || !window.firebase.firestore) {
          console.error("Firebase ERROR: SDK non completamente disponibile globalmente.");
          setIsAuthReady(true);
          return;
        }
        ...
        console.error("Errore durante l'inizializzazione Firebase:", error);
        setIsAuthReady(true);
      }
    };
